...
ctrx <- correlatrix <-
        function(data,
                ...
                if (any(c("imputationList", "amelia") %in% class(data))) {
                        if (Sys.Date() < as.Date("2018-11-01") & method != "pearson") {
                                warning(
                                        "NOTE: In versions 0.3.0 and 0.4.0 a bug resulted in the method argument not being handled appropriately for multiply imputed data. This meant that during this period, regardless of the method inputted, you would have received a pearson correlation matrix (the default). This issue has now been fixed. I sincerely apologise if you were affected by this bug. This warning will self-destruct on the 1st of November",
                                        call. = F
                                )
                        }
                }
